...
class Calc:
	...
	def require_file(self):
		f = file(os.path.relpath('./data/names.txt'))
		self.names = sorted(f.read().replace('"','').split(','))
	...
	def find(self):
		...
		for idx, name in enumerate(self.names):
		    sum += (idx+1) * self.calculate_value(name)

		print sum

	def calculate_value(self, name):
		s = list(name)
		return reduce(lambda sum, i: sum+ord(i)-64, s, 0)
